...
function rats_logdensity_match_stan_for_mooncake(evaluation_env, params)
    (; alpha, xbar, sigma, alpha0, x, Y, beta) = evaluation_env
    ...
    counter = 6
    for i in 30:-1:1
        beta[i] = params[counter]
        alpha[i] = params[counter + 1]
        counter += 2
    end

    log_density += sum(logpdf.(dnorm(alpha_c, alpha_tau), alpha))
    log_density += sum(logpdf.(dnorm(beta_c, beta_tau), beta))

    for i in 1:30
        for j in 1:5
            log_density += logpdf(dnorm(alpha[i] + beta[i] * (x[j] - xbar), tau_c), Y[i, j])
        end
    end
